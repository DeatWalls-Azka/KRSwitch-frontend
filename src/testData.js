// Test data for the class barter system
// 150 students, all enrolled in every course and all class types
...
// Helper function to generate enrollments
function generateEnrollments() {
  const enrollments = [];
  let enrollmentId = 1;
  
  // Course structure: which classes each course has
  ...
  // Enroll each student in all courses and all class types
  users.forEach((user, userIndex) => {
    Object.entries(courseStructure).forEach(([courseCode, structure]) => {
      // Enroll in one kuliah class (distribute evenly)
      ...
      // Enroll in one praktikum class if available (distribute evenly)
      ...
      // Enroll in one responsi class if available (distribute evenly)
...
export const enrollments = generateEnrollments();

export const barterOffers = [
...
export const currentUser = users.find(u => u.nim === 'M6401211150');