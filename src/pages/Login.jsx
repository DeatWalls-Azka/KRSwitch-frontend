...
export default function Login() {
  ...
  // Kalau dah login, langsung ke dashboard
  useEffect(() => {
    getCurrentUser()
      .then(() => navigate('/'))
  ...
  // Handle error dari query param (fallback non-popup)
  useEffect(() => {
  ...
  const handleLogin = () => {
    ...
    // Tutup popup lama kalau masih ada
    ...
    // Kalau popup ditutup manual tanpa selesai login
      ...
      <div className="flex flex-col items-center gap-6">
        ...
        <div className="bg-white border border-gray-200 p-6 flex flex-col items-center gap-4 w-72">
          ...
          <button
            onClick={handleLogin}
            disabled={loading}
            className="w-full flex items-center justify-center gap-2.5 bg-white border border-gray-300 px-4 py-2.5 text-[11px] font-bold text-gray-700 hover:bg-gray-50 active:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
