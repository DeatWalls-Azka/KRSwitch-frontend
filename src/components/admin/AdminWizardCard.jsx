...
export default function AdminWizardCard({ stats, onRefresh }) {
  ...
  return (
    <Card className="h-full border-border shadow-sm rounded-md flex flex-col overflow-hidden">
      <CardHeader className="py-2.5 px-4 border-b bg-muted/5 flex flex-row items-center justify-between space-y-0">
        <CardTitle className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
          {activeStep === 0 ? 'Master Configuration' : 'System Operational Status'}
        </CardTitle>
        <div className="flex items-center gap-1.5">
          {[0, 1].map((s) => (
            <div key={s} className={`h-1 w-4 rounded-full ${activeStep === s ? 'bg-primary' : activeStep > s ? 'bg-primary/40' : 'bg-muted'}`} />
          ))}
        </div>
      </CardHeader>
                ...
                <div key={type} className="space-y-1.5">
                  ...
                  <div onDragOver={(e) => { e.preventDefault(); setDraggingType(type); }} onDragLeave={() => setDraggingType(null)} onDrop={(e) => handleDrop(e, type)}
                    className={`group relative py-7 border-2 rounded-md transition-all flex flex-col items-center justify-center space-y-1 ${
                      draggingType === type ? 'border-primary bg-primary/5' : 
        ...
        <div className="flex items-center justify-between mt-auto pt-2">
          ...
          {activeStep === 0 && (
            <Button variant="admin" size="sm" disabled={!isStep0Ready() || isProcessing} onClick={handleCommitAndRandomize} className="h-8 text-[11px] font-bold px-6">
              {isProcessing ? <Loader2 className="animate-spin" size={14} /> : (pendingFiles.students || pendingFiles.classes ? 'Commit & Generate' : 'System Ready')}
