...
interface TargetClassStudent {
  nim: string;
  name: string;
}

interface OverrideTabProps {
  student: Student;
}

// --- Komponen Utama -------------------------------------------

const OverrideTab = ({ student }: OverrideTabProps) => {
  ...
  const [targetClassStudents, setTargetClassStudents] = useState<TargetClassStudent[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      ...
      try {
        const res = await api.get<TargetClassStudent[]>(`/api/admin/classes/${targetClassId}/students`);
