...
function getClassTypeStyle(code) {
  ...
  if (p === 'K') return { bg: '#DBEAFE', border: '#3B82F6', text: '#1E3A8A', badge: '#3B82F6', label: 'KULIAH' };
  if (p === 'P') return { bg: '#FEF08A', border: '#CA8A04', text: '#713F12', badge: '#CA8A04', label: 'PRAKTIKUM' };
  if (p === 'R') return { bg: '#FEE2E2', border: '#EF4444', text: '#7F1D1D', badge: '#EF4444', label: 'RESPONSI' };
...
const HOUR_TICKS = Array.from({ length: DAY_END_HOUR - DAY_START_HOUR + 1 }, (_, i) => {
  ...
  return { hour, label: `${String(hour).padStart(2, '0')}:00`, pct: toTopPct(hour * 60),
           isFirst: i === 0, isLast: i === DAY_END_HOUR - DAY_START_HOUR };
});
...
function cardToSVG(cardEl, footerEl) {
  const rect = cardEl.getBoundingClientRect();
  const w    = Math.round(rect.width);
  const h    = Math.round(rect.height - footerEl.getBoundingClientRect().height);

  const clone = cardEl.cloneNode(true);
  inlineStyles(cardEl, clone);

  clone.querySelectorAll('*').forEach(el => {
    el.style.transition = 'none'; el.style.animation = 'none'; el.style.willChange = 'auto';
    if (el.style.position === 'fixed' || el.style.position === 'sticky') el.style.position = 'absolute';
  });

  clone.querySelector('[data-export-exclude]')?.remove();
  Object.assign(clone.style, { width: `${w}px`, height: `${h}px`, overflow: 'hidden',
...
export default function ScheduleGraphModal({
  ...
  // FIX 1: separate ref for the floating panel so proximity check covers both button + panel
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (!initialized.current) { tooltipPos.current = { ...mousePos.current }; initialized.current = true; }
      else { tooltipPos.current.x = lerp(tooltipPos.current.x, mousePos.current.x);
    ...
    if (tooltipData) { requestAnimationFrame(() => setTooltipVisible(true)); rafRef.current = requestAnimationFrame(tick); }
    else             { setTooltipVisible(false); initialized.current = false; }
    return () => cancelAnimationFrame(rafRef.current);
  }, [tooltipData]);

  // FIX 1: check proximity against BOTH the trigger button AND the floating panel
  useEffect(() => {
  ...
  const mySchedule = useMemo(() => {
    ...
    const ids = new Set(enrollments.filter(e => e.nim === currentUser.nim).map(e => e.parallelClassId));
    return parallelClasses.filter(pc => ids.has(pc.id));
  }, [enrollments, parallelClasses, currentUser]);

  const scheduleByDay = useMemo(() => {
    const map = {}; DAYS.forEach(d => { map[d.key] = []; });
  ...
  // ── Export ───────────────────────────────────────────────────────────────────
  const capture = useCallback(() => cardToSVG(cardRef.current, footerRef.current), []);
  ...
  const handleExportSVG = useCallback(() => doExport('svg', () => {
    const r = capture(); if (r) exportSVG(r.svg, 'jadwal.svg');
  }), [capture, doExport]);

  const handleExportJPG = useCallback(() => doExport('jpg', () => {
    const r = capture(); if (r) exportJPG(r.svg, r.w * 3, r.h * 3, 'jadwal.jpg');
  }), [capture, doExport]);

  const handleExportPortrait = useCallback(() => doExport('portrait', () => {
    const r = capture(); if (!r) return;
    const CW = 1080, CH = 1920;
    ...
    img.onload = () => {
      ...
      try { canvas.toBlob(b => dlBlob(b, 'jadwal-portrait.jpg'), 'image/jpeg', 0.93); }
      catch { exportSVG(wrapPortraitSVG(r.svg, r.w, r.h), 'jadwal-portrait.svg'); }
    };
    img.onerror = () => console.error('SVG rasterise failed');
  ...
  const handleClose = () => {
    setTooltipData(null); setIsClosing(true);
    setTimeout(() => { setIsClosing(false); onClose(); }, 150);
  };

  // FIX 2 & 3: proper SVG icons, consistent alignment via fixed-width icon cell
  const exportItems = [
  ...
  return (
    <>
          ...
          <div ref={cardRef}
            ...
            <div className="flex items-center justify-between px-5 py-2.5 border-b border-gray-200 bg-gray-50 flex-shrink-0">
              ...
              <div className="relative">
                ...
                {/* FIX 1: panel has its own ref — proximity check tests this separately from the button */}
                <div
                  ref={exportPanelRef}
                  role="menu"
                  ...
                  <div style={{ overflow: 'hidden', minHeight: 0 }}>
                    ...
                    <div className="py-1">
                      {exportItems.map(({ key, label, sub, icon, fn }) => (
                        ...
                        >
                          {/* Fixed-width icon cell keeps all labels horizontally aligned */}
                  ...
                  <div className="flex-shrink-0 w-14 flex flex-col">
                    ...
                    <div className="relative flex-1" style={{ paddingTop: 8, paddingBottom: 8 }}>
                      {HOUR_TICKS.map(({ hour, label, pct, isFirst, isLast }) => (
                        <div key={hour} className="absolute w-full flex justify-end pr-2"
                        ...
                        <div key={key} className="flex-1 min-w-0 flex flex-col overflow-hidden">
                          ...
                          <div className="relative flex-1">
                            ...
                            {HOUR_TICKS.slice(0, -1).map(({ hour }) => (
                              <div key={`${hour}h`} className="absolute left-0 right-0"
                            ...
                            {dayClasses.map(pc => {
                              ...
                              const isTiny  = hPct >= 3;
                              return (
                                <div key={pc.id} className="absolute cursor-default select-none rounded"
                                  style={{ top: `calc(${topPct}% + 2px)`, height: `calc(${hPct}% - 4px)`,
                                  ...
                                  {!isTiny && (
                                    <div className="absolute whitespace-nowrap font-bold leading-none overflow-visible"
                                  ...
                                  {isShort && !isFull && (
                                    <div className="relative flex flex-col justify-between h-full overflow-hidden px-1.5 py-1">
                                      <span className="absolute font-bold rounded px-1 py-0.5 whitespace-nowrap"
            ...
            {/* ── Footer ── */}
            <div ref={footerRef} data-export-exclude
              className="px-5 py-2.5 border-t border-gray-200 bg-gray-50 flex items-center justify-between flex-shrink-0">
      ...
      {/* ── Tooltip ── */}
      {tooltipData && (
        <div id="sched-tip" className="fixed pointer-events-none z-[99999]"
