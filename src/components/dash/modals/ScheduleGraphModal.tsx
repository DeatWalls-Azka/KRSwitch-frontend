...
const toTopPct      = (m: number) => ((m - DAY_START_HOUR * 60) / TOTAL_MINUTES) * 100;
const toDurationPct = (m: number) => (m / TOTAL_MINUTES) * 100;
...
function inlineStyles(liveNode: Element, cloneNode: Element) {
  ...
  const cloneHtmlNode = cloneNode as HTMLElement;
  if (cloneHtmlNode.style) {
    cloneHtmlNode.style.cssText = css;
  }
  cloneHtmlNode.removeAttribute('class');
...
function cardToSVG(cardEl: HTMLElement) {
...
function exportSVG(svgStr: string, name: string) {
  dlBlob(new Blob([svgStr], { type: 'image/svg+xml;charset=utf-8' }), name);
}

function wrapPortraitSVG(svg: string, cardW: number, cardH: number) {
...
function exportJPG(svgStr: string, outW: number, outH: number, name: string) {
  const img = new Image();
  img.onload = () => {
...
interface ScheduleGraphModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ScheduleGraphModal({
  ...
  const [tooltipData, setTooltipData] = useState<ParallelClass | null>(null);
  ...
  useEffect(() => {
    const lerp = (a: number, b: number) => a + (b - a) * 0.2;
    const tick = () => {
      ...
      const tipEl = document.getElementById('sched-tip');
      if (tipEl) {
        tipEl.style.setProperty(
          'transform',
          `translate(${tooltipPos.current.x + 15}px,${tooltipPos.current.y + 15}px)`
        );
      }
  ...
  useEffect(() => {
    ...
    const handle = (e: MouseEvent) => {
      ...
      const inside = els.some(el => {
        const htmlEl = el as HTMLElement;
        if (htmlEl.offsetParent === null) return false; // skip hidden elements
  ...
  const scheduleByDay = useMemo(() => {
    const map: Record<string, ParallelClass[]> = {};
  ...
  const capture = useCallback(() => {
    ...
    return cardToSVG(cardRef.current);
  }, []);

  const doExport = useCallback((key: string, fn: () => void) => {
    setExporting(key);
  ...
  const handleExportPortrait = useCallback(() => doExport('portrait', () => {
    ...
    img.onload = () => {
      ...
      try {
        canvas.toBlob(b => {
          if (b) dlBlob(b, 'jadwal-portrait.jpg');
        }, 'image/jpeg', 0.93);
      } catch {
                        ...
                        <div key={key} className="flex-none w-[110px] sm:w-[128px] md:flex-1 md:min-w-0 flex flex-col overflow-hidden">
                          ...
                          <div className="relative flex-1">
                            ...
                            {dayClasses.map(pc => {
                              ...
                              const isFull  = validHPct >= 10;
                              const isShort = validHPct >= 5;
                              const isTiny  = validHPct >= 2;
                                ...
                                >
                                  ...
                                  {isFull && (
                                    <div className="relative flex flex-col h-full overflow-hidden px-1.5 py-1.5">
                                      ...
                                      <div className="opacity-55 truncate mt-0.5" style={{ fontSize: '10px' }}>
                                        {pc.courseCode}
                                      </div>
        ...
        >
          <div className="bg-gray-900 text-white text-[11px] px-3 py-2 rounded shadow-xl whitespace-nowrap">
            <div className="font-bold mb-1" style={{ fontSize: '12px' }}>
              {tooltipData.courseName || tooltipData.courseCode}
            </div>
            <div className="text-gray-300">
              {tooltipData.courseCode} · {tooltipData.classCode}
            </div>
            <div className="text-gray-400 mt-1">
              {tooltipData.timeStart} - {tooltipData.timeEnd}
            </div>
