...
// --- Subkomponen Baris Notifikasi ----------------------------

function NotificationRow({ notification, parallelClasses }: NotificationRowProps) {
  ...
  let oldClassCode = '-';
  let newClassCode = '-';
  const courseCode = data?.courseCode || data?.yourOldClass?.courseCode || '-';
  const isAdminAction = type?.startsWith('admin_');
  const isCancelled = type?.includes('cancelled');
